	...
	var PAGE = {
		...
		submit:					function() {
			{if $DisplaySegments}
				if($('#ShowSegmentOptions').get(0).checked) {
					...
					if(elm.selectedIndex == -1) {
						alert("{$lang.Addon_reengagement_Alert_ChooseSegment}");
						return;
					}
				}
			{/if}
			if ($('#ShowListOptions').get(0).checked) {
				...
				if(elm.selectedIndex == -1) {
					alert("{$lang.Addon_reengagement_Alert_ChooseList}");
					return;
				}
			}
...
<form name="frmSend" method="post" action="{$AdminUrl}&Action=Send&Step=2">
	<table cellspacing="0" cellpadding="0" width="100%" align="center">
		<tr>
			<td class="Heading1">
				{$lang.Addon_reengagement_Send_Step1}
			</td>
		</tr>
		<tr>
			<td class="body pageinfo">
				<p>
					{$lang.Addon_reengagement_Send_Step1_Intro}
				</p>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<table border="0" cellspacing="0" cellpadding="2" width="100%" class="Panel">
					<tr>
						<td colspan="2" class="Heading2">
							&nbsp;&nbsp;{$lang.Addon_reengagement_Send_Step1_WhoToSendTo_Heading}
						</td>
					</tr>
					<tr>
						<td class="FieldLabel">
							{template="not_required"}
							{$lang.Addon_reengagement_Send_Step1_WhoToSendTo}&nbsp;
						</td>
						<td valign="top">
							<table width="100%" cellspacing="0" cellpadding="0">
								<tr>
									<td>
										...
										id="ShowListOptions" class="SendFilteringOption" value="1" checked="checked" />{$lang.Addon_reengagement_Send_Step1_WhoToSendTo_Lists}</label>
									</td>
								</tr>
								{if $DisplaySegments}
								<tr>
									<td>
										...
										class="SendFilteringOption" value="2" />{$lang.Addon_reengagement_Send_Step1_WhoToSendTo_Segments}</label>
									</td>
								</tr>
								{/if}
							</table>
				...
				<div id="FilteringOptions">
					<table border="0" cellspacing="0" cellpadding="2" width="100%" class="Panel">
						<tr>
							<td colspan="2" class="Heading2">
								&nbsp;&nbsp;{$lang.Addon_reengagement_Send_Step1_List_Heading}
							</td>
						</tr>
						<tr>
							<td width="200" class="FieldLabel">
								{template="not_required"}
								{$lang.Addon_reengagement_Send_Step1_ChooseList}&nbsp;
							</td>
							<td>
								...
								{foreach from=$user_lists key=k item=list}
									...
									{$list.name}
									({if $list.subscribecount == 1}{$lang.Addon_reengagement_SingleContact}{else}{$list.subscribecount|number_format}{$lang.Addon_reengagement_MultipleContacts}{/if})
									</option>
								{/foreach}
								</select>
							</td>
				...
				<div id="SegmentOptions" style="display:none;">
					<table border="0" cellspacing="0" cellpadding="2" width="100%" class="Panel">
						<tr>
							<td colspan="2" class="Heading2">
								&nbsp;&nbsp;{$lang.Addon_reengagement_Send_Step1_Segment_Heading}
							</td>
						</tr>
						<tr>
							<td width="200" class="FieldLabel">
								{template="not_required"}
								{$lang.Addon_reengagement_Send_Step1_ChooseSegment}&nbsp;
							</td>
							<td>
